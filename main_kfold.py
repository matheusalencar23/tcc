...
def aptidao(x, i):
    ...
    scores = []
    for k, (i_treino, i_teste) in enumerate(kf.split(X, Y)):
        regr = MLPRegressor(random_state=1, learning_rate_init=learning_rate_init, shuffle=True,
                            ...
                            early_stopping=True, validation_fraction=0.1).fit(X[i_treino], Y[i_treino])
        scores.append(regr.score(X[i_teste], Y[i_teste]))
