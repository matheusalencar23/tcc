...
x_total = np.c_[velocidade, temperatura, preenchimento, espessura, orientacao]
y_total = resistencia
x_treino, x_teste, y_treino, y_teste = train_test_split(
    x_total, y_total, train_size=TAM_TREINO, random_state=1)
...
def aptidao(x, i):
    ...
    score = 0
    for i_treino, i_teste in kf.split(x_total, y_total):
        regr = MLPRegressor(random_state=1, learning_rate_init=learning_rate_init, shuffle=True,
                            ...
                            early_stopping=False, validation_fraction=0.1).fit(x_total[i_treino], y_total[i_treino])
        score += regr.score(x_total[i_teste], y_total[i_teste])
    return score
